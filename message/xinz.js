...
//------------------< Sticker / Tools >-------------------
            ...
            case prefix+'attp':{
                ...
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/ttp?apikey=${lolkey}&text1=${q.split("|")[0]}&text2=${q.split("|")[1]}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
            case prefix+'tinyurl':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
...
//------------------< NULIS >---------------------
            ...
            case prefix+'foliokanan':{
                ...
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/ktpmaker?apikey=${lolkey}&text=${q}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'ocr':
                if (!isPremium) return reply(`Kamu bukan user premium, kirim perintah *${prefix}daftarprem* untuk membeli premium`)
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/ocr?apikey=${lolkey}&text=${q}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'ayatalquranmp3':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/quran/audio/18/1?apikey=${lolkey}&text=${q}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                ...
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/asmaulhusna?apikey=${lolkey}&text=${q}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'ayatalquran':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/quran/18?apikey=${lolkey}&text=${q}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
//------------------< Text Marker >-------------------
...
//------------------< INFO >-------------------
            ...
            case prefix+'donate': case prefix+'donasi':
                ...
                break
            case prefix+'script': case prefix+'sc': case prefix+'src':
...
//------------------< VVIBU >-------------------
            ...
            case prefix+'sagiri':
                ...
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/random/nsfw/ecchi?apikey=${lolkey}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'hentai':
                if (!isPremium) return reply(`Kamu bukan user premium, kirim perintah *${prefix}daftarprem* untuk membeli premium`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/random/nsfw/hentai?apikey=${lolkey}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'nhentaipdf':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/nhentaipdf?apikey=${lolkey}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'animeapani':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/wait?apikey=${lolkey}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
                case prefix+'mangaapani':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                sendFileFromUrl(from, `https://api.lolhuman.xyz/api/wmit?apikey=${lolkey}`, '', msg).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
//------------------< Premium >-------------------
            case prefix+'addprem':
